...
export default function InstitutionsPage() {
  ...
  return (
    <main className="mx-auto max-w-5xl p-5 sm:p-6">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-[22px] font-bold leading-tight text-zinc-900">Institutions</h1>
        <p className="mt-0.5 text-[13px] text-zinc-500">
          Manage institutions. Their names and logos auto-fill on certificates.
        </p>
      </div>
      ...
      <div className="mb-4 overflow-hidden rounded-2xl border border-zinc-100 bg-white shadow-[0_1px_4px_rgba(0,0,0,0.04),0_1px_2px_rgba(0,0,0,0.02)]">
        <div className="border-b border-zinc-100 px-5 py-3">
          <p className="text-[10px] font-semibold uppercase tracking-[0.08em] text-zinc-400">Add Institution</p>
        </div>
        <div className="p-5">
          {addError && (
          ...
          <form
            onSubmit={async (event) => {
              ...
              setAddError(null);
              try {
                await adminApi.createInstitution({ ...form, is_active: true });
                setForm({ name: "", website_url: "", logo_url: "" });
                await load();
              } catch (e) {
                setAddError(e instanceof Error ? e.message : "Failed to add institution");
              }
            }}
            className="grid gap-3 sm:grid-cols-[1fr_1fr_1fr_auto]"
          >
            <div>
              <label className="mb-1 block text-[11px] font-medium text-zinc-400">Institution Name *</label>
              <input
                className={inputCls}
            ...
            <div>
              <label className="mb-1 block text-[11px] font-medium text-zinc-400">Website URL</label>
              <input
                className={inputCls}
            ...
            <div>
              <label className="mb-1 block text-[11px] font-medium text-zinc-400">Logo URL / Path</label>
              <input
                className={inputCls}
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-zinc-100 bg-zinc-50/80">
                ...
                <th className="hidden px-4 py-3 text-[10px] font-semibold uppercase tracking-[0.08em] text-zinc-400 md:table-cell">Website</th>
                <th className="hidden px-4 py-3 text-[10px] font-semibold uppercase tracking-[0.08em] text-zinc-400 lg:table-cell">Logo</th>
                ...
                <tr key={item.id} className="transition-colors hover:bg-zinc-50/70">
                  ...
                  <td className="hidden max-w-[200px] px-4 py-3 md:table-cell">
                    {editId === item.id ? (
                      ...
                      <a
                        ...
                        className="flex items-center gap-1 text-[12px] text-blue-600 transition hover:underline"
                      >
                  ...
                  <td className="hidden max-w-[180px] px-4 py-3 lg:table-cell">
                    {editId === item.id ? (
                      ...
                      <span className="truncate text-[12px] text-zinc-500">{item.logo_url}</span>
                    ) : (
                      <span className="text-zinc-300">—</span>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      {editId === item.id ? (
                        <>
                          <button
                            ...
                            onClick={async () => {
                              if (!draft) return;
                              await adminApi.updateInstitution(item.id, draft);
                              setEditId(null);
                              setDraft(null);
                              await load();
                            }}
                      ...
                      <button
                        ...
                        onClick={async () => {
                          await adminApi.updateInstitution(item.id, { ...item, is_active: !item.is_active });
                          await load();
                        }}
